...
namespace MVCPeopleAwards.Repositories
{
    public class PeopleRepository : IDisposable, IRepositoryPeople
    {
        ...
        private void PeopleMapToPeopleModel(People source, ref PeopleViewModel dest)
        {
            try
            {
                dest.Id = source.Id;
        ...
        public int SavePeopleAward(int peopleID, int awardID)
        {
            ...
            {
                dbContext.SaveChanges();
                return dbContext.ListPeopleAwards.ToList().FirstOrDefault(c => c.AwardID == awardID && c.PeopleID == peopleID).Id;
            }
            catch (Exception ex)
            {
                Logger.LogException(new Exception(String.Format("Ошибка:\n{0}\n{1}\n{2}", ex.Message, ex.StackTrace, ex.InnerException.StackTrace)));
